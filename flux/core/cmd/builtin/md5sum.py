...
class Command(CommandInterface):
    ...
    def init(self):
        self.parser = Parser(prog="md5sum", description="Print or check MD5 (128-bit) checksums")
        self.parser.add_argument("FILE", nargs="*", help="With no FILE, or when FILE is -, read standard input.")
        self.parser.add_argument("-c", "--check", action="store_true", help="read MD5 sums from the FILE(s) and check them")
        ...
        if self.args.check:
            ...
            for file_path in files:
                if file_path == "-":
                    ...
                    if self.args.tag:
                        self.print(f"MD5 (-) = {md5.hexdigest()}")
                    else:                    
                        self.print(f"{md5.hexdigest()}  -")
                else:
                    ...
                    if self.args.tag:
                        self.print(f"MD5 ({file_path}) = {digest}")
                    else:                    
