...
class Command(CommandInterface):
    ...
    def init(self):
        self.parser = Parser(prog="sha1sum", description="Print or check SHA1 (160-bit) checksums")
        self.parser.add_argument("FILE", nargs="*", help="With no FILE, or when FILE is -, read standard input.")
        self.parser.add_argument("-c", "--check", action="store_true", help="read SHA1 sums from the FILE(s) and check them")
        ...
        if self.args.check:
            ...
            for file_path in files:
                if file_path == "-":
                    ...
                    if self.args.tag:
                        self.print(f"SHA1 (-) = {sha1.hexdigest()}")
                    else:                    
                        self.print(f"{sha1.hexdigest()}  -")
                else:
                    ...
                    if self.args.tag:
                        self.print(f"SHA1 ({file_path}) = {digest}")
                    else:                    
                        self.print(f"{digest}  {file_path}")


    def _hash_file(self, path: str) -> str:
        """Compute SHA1 hash of a file in binary mode."""
        sha1 = hashlib.sha1()
