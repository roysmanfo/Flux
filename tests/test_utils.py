...
class Test_Utils(unittest.TestCase):
    ...
    def test_returns_a_list(self):
        test = "   Hello   World!   "
        formatted = parsing.string_to_list(test)
...
class Test_StringToList(unittest.TestCase):
    ...
    def test_03(self):
        cmd = parsing.string_to_list("LS -h")
        self.assertTrue(cmd == ["ls", "-h"], cmd)

    def test_04(self):
        cmd = parsing.string_to_list("LS -R")
        self.assertTrue(cmd == ["ls", "-R"], cmd)

    def test_05(self):
        cmd = parsing.string_to_list("ls >> file")
        self.assertTrue(cmd == ["ls", ">>", "file"], cmd)

    def test_06(self):
        cmd = parsing.string_to_list("ls 2>/dev/null")
        self.assertTrue(cmd == ["ls", "2>", "/dev/null"], cmd)

    def test_07(self):
        cmd = parsing.string_to_list("LS << Input.txt")
        self.assertTrue(cmd == ["ls", "<<", "Input.txt"], cmd)

    def test_08(self):
        cmd = parsing.string_to_list("echo test | ls")
...
class Test_CommandSeparator(unittest.TestCase):
    ...
    def test_01(self):
        cmd = parsing.split_commands("echo test ; ls")
        self.assertTrue(cmd == [["echo", "test"], ["ls"]], cmd)
    ...
    def test_10(self):
        cmd = parsing.split_commands("echo test;ls  ..;echo null")
        self.assertTrue(cmd == [["echo", "test"], ["ls", ".."], ["echo", "null"]], cmd)

    def test_11(self):
        cmd = parsing.split_commands("echo test | cat")
        self.assertTrue(cmd == [["echo", "test", "|", "cat"]], cmd)

    def test_11(self):
        cmd = parsing.split_commands("echo test | cat; ls ..")
...
class Test_PipeSeparator(unittest.TestCase):
    def test_01(self):
        cmd = parsing.split_pipe("echo test | ls")
        self.assertTrue(cmd == [["echo", "test"], ["ls"]], cmd)
